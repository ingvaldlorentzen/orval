...
describe('collapseMswFakerFullOutputs', () => {
  ...
  describe('MSW-only (no Faker configured)', () => {
    it('strips function portion from MSW output', () => {
      const outputs = [mswFull()];
    ...
    it('replaces ResponseMock fallback calls with undefined in handler', () => {
      const outputs = [
        mswFull({
          implementation: {
            function: 'export const getPetResponseMock = () => ({})\n',
    ...
    it('replaces ResponseMock fallback calls with status suffixes', () => {
      const outputs = [
        mswFull({
          implementation: {
            function:
    ...
    it('preserves non-faker imports', () => {
      const outputs = [mswFull()];
      const result = collapseMswFakerFullOutputs(outputs);
